...
Размер одной записи - около 424 байт.

При размере аудитории в 75 млн пользователей получим общий объём таблицы: 29.6 Гб
...
| ------------------- | ------------------- | ------------ | ------------ | ------------------ | ------------------ |
| User                | 192 Байта           | 75 млн       | 13.4 Гб      | 39618              | 0.66               |
...
Самым сложным алгоритмом в MVP Tinder является локальная система рекомендаций, которая основана на динамическом определении геопозиции пользователей и подбору возможных пар с похожей геопозицией. Геопозиция пользователя в течении короткого времени хранится в бд. Для таких целей подойдет Cassandra, так как она имеет встроенные пространственные индексы и может быстро возвращать результаты с ближайшими пользователями, а надежностью хранения данных можно пренебречь.
