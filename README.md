...
Сохранять сообщения батчами по 1 млн строк с учетом того, чтобы они хранились вместе со своим чатом.
...
Репликация ClickHouse: настроить несколько реплик и использовать их для чтения

Горячее резервирование ClickHouse: реализовать мастер-мастер репликацию между серверами, чтобы каждый из них мог выступать как в роли мастера, так и в роли резервного сервера. В случае сбоя одного из серверов другой может взять на себя его нагрузку без прерывания работы приложения.

Репликация Kafka: один брокер выступает в роли лидера (мастера), а остальные следуют за ним (слейвы). Слейвы синхронно реплицируют сообщения с мастера. Replication factor = 3.

## 10. Схема проекта<a name="10"></a>

![Схема проекта](sheme.jpg)
...
| ---------- | ------- | -------------------------------------------------------- | ----- | --- | ----------------- | ------------------------------------ |
| kube node  | own     | 2xAMD EPYC 7773/16x32GB/3200MHz DDR4/1xNVMe256GB         | 64    | 11  | 16 000            | 267                                  |
| MongoDB    | own     | 2xAMD EPYC 7763/16x32GB/3200MHz DDR4/1xNVMe128GB         | 64    | 7   | 19 000            | 317                                  |
| ClickHouse | own     | 2xAMD EPYC 7H12/16x64GB/3200MHz DDR4/12x4TBNVMe          | 64    | 11  | 65 000            | 1083                                 |
| Redis      | own     | 1xIntel Xeon E-2388G/1x32GB/3200MHz DDR4/2x240GB SATA SSD| 8     | 1   | 2 150             | 36                                   |
| S3         | own     | 2xAMD EPYC 7742/16x32GB/8xNVMe8T/3200MHz DDR4/2x960GBNVMe| 64    | 8   | 63 000            | 1050                                 |
