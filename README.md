...
1. На входе в ДЦ ставим чётное число роутеров (для обеспечения отказоустойчивости)
2. Далее трафик мобильный проксируется на nginx через BGP, а веб запросы балансируются напрямую на application-сервера
